...
test.describe("Garden planner sandbox", () => {
  ...
  test("planting popovers stay within bed bounds at every edge", async ({ page }) => {
    ...
    for (const markerId of markerIds) {
      ...
      expect(popBox.left).toBeGreaterThanOrEqual(bedBox.left - 2);
      expect(popBox.right).toBeLessThanOrEqual(bedBox.right + 2);
      expect(popBox.top).toBeGreaterThanOrEqual(bedBox.top - 2);
      expect(popBox.bottom).toBeLessThanOrEqual(bedBox.bottom + 2);
